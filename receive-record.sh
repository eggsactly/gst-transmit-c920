...
gst-launch-1.0 -v \
	udpsrc multicast-group=224.1.1.1 auto-multicast=true port=5001 ! application/x-rtp ! rtpjitterbuffer ! rtph264depay ! tee name=v \
	udpsrc multicast-group=224.1.1.1 port=5002 caps="application/x-rtp,channels=(int)2,media=(string)audio,payload=(int)96,clock-rate=(int)32000" ! rtpjitterbuffer ! rtppcmudepay ! queue ! mulawdec ! tee name=a \
